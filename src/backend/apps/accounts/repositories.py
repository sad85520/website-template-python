...
class IUserRepository(Protocol):
    """使用者資料存取的結構型別協議，定義 repository 的最小介面。"""
...
class UserRepository:
    ...
    def get_by_id(self, user_id: object) -> User | None:
        ...
        Args:
            user_id: 使用者主鍵（UUID 或字串）。

        Returns:
            找到則回傳 User；否則回傳 None。
        """
    ...
    def get_all(self) -> db_models.QuerySet[User]:
        """回傳所有使用者的 QuerySet（依 Meta.ordering 預設排序）。
    ...
    def search(self, query: str) -> db_models.QuerySet[User]:
        """依關鍵字搜尋使用者（比對 email 與 display_name）。

        Args:
            query: 搜尋關鍵字；為空字串時等同 get_all()。
